...
let _solAlertShown = false;
let _solAlertInterval = null;
let _solLastCount = 0;

function _showSolAlert(count, items) {
  // Não mostrar se já existe um modal aberto
  if (document.getElementById('modal-sol-alert')) return;

  const modal = document.createElement('div');
  modal.id = 'modal-sol-alert';
  modal.style.cssText = `
    position:fixed;bottom:24px;right:24px;z-index:99999;
    width:320px;background:var(--bg2);border:1px solid var(--warn);
    border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,.35);
    overflow:hidden;animation:slideInRight .3s cubic-bezier(.22,1,.36,1);
  `;

  const preview = (items||[]).slice(0,3).map(s =>
    `<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--bdr)">
      ...
      <div style="font-size:11px;color:var(--t1);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(s.title||s.description||'Solicitação #'+s.id)}</div>
      <div style="font-size:10px;color:var(--t3);flex-shrink:0">${s.requester_name||''}</div>
    </div>`
  ).join('');

  modal.innerHTML = `
    <div style="background:rgba(234,179,8,.1);padding:14px 16px;display:flex;align-items:center;gap:10px;border-bottom:1px solid var(--bdr)">
      <div style="width:36px;height:36px;border-radius:10px;background:rgba(234,179,8,.15);display:flex;align-items:center;justify-content:center;flex-shrink:0">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--warn)" stroke-width="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
      </div>
      <div style="flex:1">
        <div style="font-weight:700;font-size:13px;color:var(--t1)">${count} Solicitaç${count===1?'ão':'ões'} Pendente${count===1?'':'s'}</div>
        <div style="font-size:11px;color:var(--t3);margin-top:1px">Aguardando sua atenção</div>
      </div>
      <button onclick="document.getElementById('modal-sol-alert')?.remove()" style="background:none;border:none;cursor:pointer;color:var(--t3);padding:4px;border-radius:6px;display:flex;align-items:center">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
      </button>
    </div>
    <div style="padding:12px 16px">
      ${preview}
      ${items && items.length > 3 ? `<div style="font-size:10px;color:var(--t3);padding-top:6px">+ ${items.length - 3} outras...</div>` : ''}
    </div>
    <div style="padding:10px 16px;display:flex;gap:8px;border-top:1px solid var(--bdr)">
      <button onclick="showPage('solicitacoes');document.getElementById('modal-sol-alert')?.remove()" style="
        flex:1;padding:8px;border:none;border-radius:8px;cursor:pointer;
        background:var(--warn);color:#000;font-weight:700;font-size:12px;
      ">Ver Solicitações</button>
      <button onclick="document.getElementById('modal-sol-alert')?.remove()" style="
        padding:8px 12px;border:1px solid var(--bdr);border-radius:8px;cursor:pointer;
  ...
  setTimeout(() => {
    const m = document.getElementById('modal-sol-alert');
    if (m) { m.style.animation = 'fadeOut .3s ease forwards'; setTimeout(() => m.remove(), 300); }
  }, 12000);
}

async function checkSolicitacoesPendentes(force) {
  // Apenas admin/diretor veem alerta de solicitações
  if(!IS_ADMIN && !IS_DIR) return;
  try {
    const sr = await api('solicitations');
    const pending = (sr||[]).filter(s => s.status === 'Pendente');
    const count = pending.length;

    // Atualizar badge sidebar
    const bSol = document.getElementById('b-sol');
    if (bSol) { bSol.textContent = count; bSol.style.display = count > 0 ? '' : 'none'; }

    // Mostrar modal se: tem pendentes E (é primeira vez OU contagem aumentou OU forçado)
    if (count > 0 && (force || count > _solLastCount)) {
      _showSolAlert(count, pending);
    }
    _solLastCount = count;
  } catch(e) {}
}
...
const cmts=(d.comments||[]).map(c=>{const isMine=(c.user_id==ME.id||c.user_name===ME.name);const canDel=isMine||IS_ADMIN;return`<div class="comment${isMine?' mine':''}" style="position:relative"><div class="ch"><span class="ca">${esc(c.user_name)}</span><span class="ct" title="${fmtDT(c.created_at)}">${timeAgo(c.created_at)}</span></div><div class="cx">${fmtMention(esc(c.text))}</div>${canDel?`<button onclick="event.stopPropagation();deleteComment(${c.id},${d.id})" title="Excluir" style="position:absolute;top:6px;${isMine?'left:-8px':'right:-8px'};background:var(--bg2);border:1px solid var(--bdr);color:var(--t3);cursor:pointer;width:20px;height:20px;border-radius:50%;display:none;align-items:center;justify-content:center;font-size:10px;transition:.2s;z-index:2" onmouseover="this.style.color='var(--err)';this.style.borderColor='var(--err)'" onmouseout="this.style.color='var(--t3)';this.style.borderColor='var(--bdr)'">&times;</button>`:''}</div>`}).join('');
...
function openReviewModal(id){
  ...
  const _detDeadline=document.querySelector('.det-info-grid')?.querySelector('[title="deadline"]')?.textContent||'';
...
async function submitReview(){
  ...
  const _revId=document.getElementById('rev-demand-id').value;
  const _revData=await api('demand',{params:{id:_revId}});
  const _revDeadline=_revData?.deadline;
  const _revAtrasado=_revDeadline&&new Date(_revDeadline+'T23:59:59')<new Date();
  if(_revAtrasado&&!obs){showToast('⚠️ Demanda atrasada! Justificativa obrigatória.');if(btn){btn.disabled=false;btn.innerHTML='Confirmar Envio'}return}
...
function updatePontoUI(){
  const el=document.getElementById('ponto-time');
  const icon=document.getElementById('ponto-icon');
  const fill=document.getElementById('ponto-fill');
  const btn=document.getElementById('ponto-btn');
  const timer=document.getElementById('ponto-timer');
  if(!el)return;
  const h=Math.floor(_pontoTotalSec/3600);
  const m=Math.floor((_pontoTotalSec%3600)/60);
  const maxSec=_pontoWorkHours*3600;
  const pct=Math.min(100,Math.round((_pontoTotalSec/maxSec)*100));
  const isExtra=_pontoTotalSec>maxSec;
  const extraSec=isExtra?_pontoTotalSec-maxSec:0;
  const eh=Math.floor(extraSec/3600);
  const em=Math.floor((extraSec%3600)/60);
  el.textContent=h+'h'+String(m).padStart(2,'0')+'m';
  if(isExtra){
    el.textContent+=(' +'+eh+'h'+String(em).padStart(2,'0')+'m');
    timer.style.background='rgba(239,68,68,.15)';
    timer.style.border='1px solid rgba(239,68,68,.3)';
    fill.style.background='var(--err)';
    fill.style.width='100%';
    icon.textContent=_pontoActive?'🔴':'⏸';
  }else{
    timer.style.background=_pontoActive?'rgba(16,185,129,.1)':'rgba(100,100,100,.1)';
    timer.style.border='1px solid '+(_pontoActive?'rgba(16,185,129,.2)':'rgba(100,100,100,.2)');
    fill.style.background=pct>=80?'var(--warn)':'var(--ok)';
    fill.style.width=pct+'%';
    icon.textContent=_pontoActive?'🟢':'⏸';
  }
  btn.textContent=_pontoActive?'Pausar':'Retomar';
  btn.style.color=_pontoActive?'var(--warn)':'var(--ok)';
}

async function togglePonto(){
  if(_pontoActive){
    const r=await api('ponto_clock_out',{method:'POST',body:{note:''}});
...
setInterval(()=>{
  if(_pontoActive)_pontoTotalSec++;
  updatePontoUI();
},1000);

// Sync com servidor a cada 60s
setInterval(async()=>{
  if(document.hidden)return;
  const st=await api('ponto_status');
  if(st){_pontoTotalSec=st.total_seconds||0;_pontoActive=st.active;_pontoWorkHours=st.work_hours||6}
},60000);

async function loadPonto(){
  const from=document.getElementById('ponto-from')?.value||new Date(Date.now()-30*864e5).toISOString().split('T')[0];
  const to=document.getElementById('ponto-to')?.value||new Date().toISOString().split('T')[0];
  const uid=IS_ADMIN?(document.getElementById('ponto-user')?.value||ME.id):ME.id;
  const[hist,allToday]=await Promise.all([
    api('ponto_history',{params:{from,to,user_id:uid}}),
    IS_ADMIN?api('ponto_all_today'):null
  ]);
  let html='';
  // Status atual
  const maxSec=_pontoWorkHours*3600;
  const pct=Math.min(100,Math.round((_pontoTotalSec/maxSec)*100));
  const h=Math.floor(_pontoTotalSec/3600),m=Math.floor((_pontoTotalSec%3600)/60);
  const isExtra=_pontoTotalSec>maxSec;
  const extraSec=isExtra?_pontoTotalSec-maxSec:0;
  
  html+=`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin-bottom:20px">
    <div class="sc blue"><div class="sc-l">Hoje</div><div class="sc-v">${h}h ${String(m).padStart(2,'0')}m</div></div>
    <div class="sc ${isExtra?'red':'green'}"><div class="sc-l">${isExtra?'Hora Extra':'Restante'}</div><div class="sc-v">${isExtra?Math.floor(extraSec/3600)+'h '+Math.floor((extraSec%3600)/60)+'m':Math.floor((maxSec-_pontoTotalSec)/3600)+'h '+Math.floor(((maxSec-_pontoTotalSec)%3600)/60)+'m'}</div></div>
    <div class="sc purple"><div class="sc-l">Jornada</div><div class="sc-v">${_pontoWorkHours}h/dia</div></div>
    <div class="sc" style="border-left:3px solid ${_pontoActive?'var(--ok)':'var(--t3)'}"><div class="sc-l">Status</div><div class="sc-v">${_pontoActive?'🟢 Ativo':'⏸ Pausado'}</div></div>
  </div>`;
  
  // Filtros
  html+=`<div style="display:flex;gap:8px;align-items:center;margin-bottom:16px;flex-wrap:wrap">
    <input type="date" class="fsel" id="ponto-from" value="${from}" onchange="loadPonto()">
    <span style="color:var(--t3)">até</span>
    <input type="date" class="fsel" id="ponto-to" value="${to}" onchange="loadPonto()">
    ${IS_ADMIN?`<select class="fsel" id="ponto-user" onchange="loadPonto()"><option value="${ME.id}">Meu Ponto</option>${allDevs.map(d=>`<option value="${d.id}" ${d.id==uid?'selected':''}>${esc(d.name)}</option>`).join('')}</select>`:''}
  </div>`;
  
  // Admin: visao geral de hoje
  if(IS_ADMIN&&allToday){
    html+=`<div class="tbl-c" style="margin-bottom:16px"><div class="tbl-bar" style="background:var(--accg)"><h3>Equipe Hoje</h3></div><div style="overflow-x:auto"><table><thead><tr><th>Dev</th><th>Status</th><th>Trabalhado</th><th>Jornada</th><th>Progresso</th></tr></thead><tbody>`;
    allToday.forEach(u=>{
      const ts=u.total_seconds||0;const wh=u.work_hours||6;const ws=wh*3600;
      const uh=Math.floor(ts/3600),um=Math.floor((ts%3600)/60);
      const upct=Math.min(100,Math.round((ts/ws)*100));
      const extra=ts>ws;
      html+=`<tr><td><div class="dev-tag">${av(u.name,u.avatar_color,22,u.avatar_file)} ${esc(u.name)}</div></td>
        <td>${u.active>0?'<span style="color:var(--ok)">🟢 Ativo</span>':'<span style="color:var(--t3)">⏸ Pausado</span>'}</td>
        <td style="font-family:'JetBrains Mono',monospace;color:${extra?'var(--err)':'var(--acc)'}">${uh}h ${String(um).padStart(2,'0')}m${extra?' <span style="color:var(--err);font-size:10px">(extra)</span>':''}</td>
        <td>${wh}h</td>
        <td><div style="display:flex;align-items:center;gap:6px"><div style="flex:1;height:6px;background:var(--bg4);border-radius:3px;overflow:hidden"><div style="height:100%;width:${upct}%;background:${extra?'var(--err)':upct>=80?'var(--warn)':'var(--ok)'};border-radius:3px"></div></div><span style="font-size:10px;font-weight:700">${upct}%</span></div></td></tr>`;
    });
    html+=`</tbody></table></div></div>`;
  // Iniciar timer tempo real para equipe
  setTimeout(_startTeamTimer, 100);
  }
  
  // Historico
  const days=hist?.days||[];
  html+=`<div class="tbl-c"><div class="tbl-bar"><h3>Histórico de Ponto</h3></div><div style="overflow-x:auto"><table><thead><tr><th>Data</th><th>Sessões</th><th>Total</th><th>Jornada</th><th>Saldo</th>${IS_ADMIN?'<th>Ações</th>':''}</tr></thead><tbody>`;
  if(days.length){
    days.forEach(day=>{
      const ts=day.total_seconds||0;const wh=day.work_hours||6;const ws=wh*3600;
      const dh=Math.floor(ts/3600),dm2=Math.floor((ts%3600)/60);
      const saldo=ts-ws;const sh=Math.floor(Math.abs(saldo)/3600),sm=Math.floor((Math.abs(saldo)%3600)/60);
      const sessoes=day.sessions.map(s=>{
        const ci=s.clock_in?.substring(11,16)||'--:--';
        const co=s.clock_out?.substring(11,16)||'ativo';
        return ci+' → '+co+(s.note?' ('+esc(s.note)+')':'')+(s.editor_name?' <span style="font-size:9px;color:var(--warn)">editado por '+esc(s.editor_name)+'</span>':'');
      }).join('<br>');
      html+=`<tr>
        <td style="font-weight:600;white-space:nowrap">${fmtDate(day.date)}</td>
        <td style="font-size:11px;line-height:1.8">${sessoes}</td>
        <td style="font-family:'JetBrains Mono',monospace;font-weight:700">${dh}h ${String(dm2).padStart(2,'0')}m</td>
        <td>${wh}h</td>
        <td style="font-family:'JetBrains Mono',monospace;color:${saldo>=0?'var(--ok)':'var(--err)'}">
          ${saldo>=0?'+':'-'}${sh}h ${String(sm).padStart(2,'0')}m
        </td>
        ${IS_ADMIN?`<td>${day.sessions.map(s=>`<button class="btn btn-g btn-sm" onclick="editPontoSession(${s.id},'${s.clock_in}','${s.clock_out||''}','${esc(s.note||'')}')" style="font-size:9px;padding:2px 6px">${IC.edit}</button>`).join(' ')}</td>`:''}
      </tr>`;
    });
  }else{
    html+=`<tr><td colspan="${IS_ADMIN?6:5}" style="text-align:center;padding:20px;color:var(--t3)">Nenhum registro no período</td></tr>`;
  }
  html+=`</tbody></table></div></div>`;
  
  document.getElementById('ponto-content').innerHTML=html;
}
...
if(!window._solInterval){window._solInterval=setInterval(()=>{const solEl=document.getElementById('sol-content');if(solEl&&solEl.offsetParent!==null&&!document.getElementById('sol-f-search')?.matches(':focus'))loadSolicitations()},10000)}
...
const avgHrs=ov.avg_days?parseFloat(ov.avg_days)*24:0;const avgHrsTxt=avgHrs?avgHrs<1?Math.round(avgHrs*60)+'min':avgHrs.toFixed(1)+'h':'—';
html+='<div class="sc purple"><div class="sc-l">Tempo Médio</div><div class="sc-v">'+avgHrsTxt+'</div></div>';
...
// WORKING NOW section
html+='<div class="tbl-c" data-chart="working" style="margin-top:14px"><div class="tbl-bar" style="background:var(--accg)"><h3 style="display:flex;align-items:center;gap:8px"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--acc)" stroke-width="2"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg> Trabalhando Agora</h3></div><div style="overflow-x:auto"><table><thead><tr><th>Demanda</th><th>Sistema</th><th>Dev(s)</th><th>Prioridade</th><th>Em execução há</th><th>Prazo</th></tr></thead><tbody>';
const workingNow=(byDev||[]).reduce((acc,d)=>acc.concat((d.demands||[]).filter(dm=>['Em Andamento','Em Revisão'].includes(dm.status))),[]);
// Also get from general - fetch active demands
const _wParams={status:'Em Andamento'};
if(fDev)_wParams.dev_id=fDev;if(fSys)_wParams.system_id=fSys;if(fSprint)_wParams.sprint_id=fSprint;
const _workAll=await api('demands',{params:_wParams})||[];
if(_workAll.length){_workAll.forEach(dm=>{
  const devNames=(dm.devs||[]).map(dv=>av(dv.name,dv.avatar_color,20,dv.avatar_file,dv.role)+' '+esc(dv.name)).join(', ');
  let timeWorking='—';
  if(dm.started_at||dm.start_date){try{const _sd=dm.started_at||dm.start_date;const s=new Date(String(_sd).replace(/ /g,'T'));if(!isNaN(s.getTime())){const diffMs=Math.max(0,Date.now()-s);const wh=(dm.devs&&dm.devs[0]&&dm.devs[0].work_hours)?dm.devs[0].work_hours:6;const totalMin=Math.floor(diffMs/6e4);const workMinPerDay=wh*60;const workDays=Math.floor(totalMin/(24*60));const workHrs=workDays*wh+Math.min(Math.floor((totalMin%(24*60))/60),wh);const wMins=totalMin%(60);timeWorking=workDays>0?workDays+'d '+Math.floor((totalMin%(24*60))/60)+'h':Math.floor(totalMin/60)>0?Math.floor(totalMin/60)+'h '+wMins+'m':wMins+'m'}}catch(e){}}
  let dlInfo='—';
  if(dm.deadline){const diff=Math.ceil((new Date(dm.deadline+'T12:00:00')-Date.now())/86400000);if(diff<0)dlInfo='<span style="color:var(--err);font-weight:700">'+Math.abs(diff)+'d atrasado</span>';else if(diff<=3)dlInfo='<span style="color:var(--err)">'+diff+'d</span>';else if(diff<=7)dlInfo='<span style="color:var(--warn)">'+diff+'d</span>';else dlInfo=diff+'d'}
  html+='<tr onclick="openDetail('+dm.id+')" style="cursor:pointer"><td style="font-weight:600">'+esc(dm.title)+'</td><td><span class="tag">'+esc(dm.system_name||'—')+'</span></td><td>'+devNames+'</td><td><span class="badge '+pClass(dm.priority)+'">'+dm.priority+'</span></td><td style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:var(--acc)">'+timeWorking+'</td><td>'+dlInfo+'</td></tr>';
})}else{html+='<tr><td colspan="6" style="text-align:center;padding:20px;color:var(--t3)">Nenhuma demanda em andamento</td></tr>'}
html+='</tbody></table></div></div>';
...
html+='<div class="chart-card" data-chart="avgdays"><h4>Tempo Médio de Conclusão (minutos)</h4><canvas id="ch-avgdays" height="250"></canvas></div>';
html+='</div>';
...
// TABLE: By System
html+='<div class="tbl-c" data-chart="bysys" style="margin-top:14px"><div class="tbl-bar"><h3>Demandas por Sistema</h3></div><div style="overflow-x:auto"><table><thead><tr><th>Sistema</th><th>Total</th><th>Abertas</th><th>Andamento</th><th>Concluídas</th><th>Tempo Médio</th></tr></thead><tbody>';
(bySys||[]).forEach(s=>{
  const _sAvgMin=s.avg_days?Math.round(parseFloat(s.avg_days)*24*60):0;const _sAvgTxt=_sAvgMin?(_sAvgMin<60?_sAvgMin+'min':Math.floor(_sAvgMin/60)+'h '+(_sAvgMin%60)+'m'):'—';html+='<tr><td style="font-weight:600">'+esc(s.name||'Sem sistema')+'</td><td style="font-weight:700">'+(s.total||0)+'</td><td>'+(s.abertas||0)+'</td><td style="color:var(--acc)">'+(s.andamento||0)+'</td><td style="color:var(--ok)">'+(s.concluidas||0)+'</td><td>'+_sAvgTxt+'</td></tr>';
});
...
html+=`<div class="tbl-c" data-chart="proximas" style="margin-top:20px">
  <div class="tbl-bar" style="background:rgba(212,160,23,.08)">
    ...
    <span class="badge" style="background:rgba(212,160,23,.15);color:#d4a017">${_aguardList.length} demanda${_aguardList.length!==1?'s':''}</span>
  </div>
  <div style="overflow-x:auto"><table>
    ...
    <tbody>`;
if(_aguardList.length){
  _aguardList.forEach(d=>{
    const devNames=(d.devs||[]).map(dv=>`<div style="display:flex;align-items:center;gap:4px">${av(dv.name,dv.avatar_color,18,dv.avatar_file,dv.role)} <span style="font-size:11px">${esc(dv.name)}</span></div>`).join('');
    let dlHtml=fmtDate(d.deadline)||'—';
    if(d.deadline){const diff=Math.ceil((new Date(d.deadline+'T12:00:00')-Date.now())/86400000);if(diff<0)dlHtml+=` <span style="color:var(--err);font-size:9px">(${Math.abs(diff)}d atraso)</span>`;else if(diff<=3)dlHtml+=` <span style="color:var(--err);font-size:9px">(${diff}d)</span>`;else if(diff<=7)dlHtml+=` <span style="color:var(--warn);font-size:9px">(${diff}d)</span>`;}
    html+=`<tr onclick="openDetail(${d.id})" style="cursor:pointer">
      ...
      <td><span class="badge ${pClass(d.priority)}">${d.priority}</span></td>
      <td>${devNames||'<span style="color:var(--t3);font-size:11px">—</span>'}</td>
      <td style="font-size:11px;font-family:'JetBrains Mono',monospace">${dlHtml}</td>
    </tr>`;
  });
}else{
  html+='<tr><td colspan="5"><div class="empty" style="padding:20px"><p>Nenhuma próxima demanda cadastrada</p></div></td></tr>';
}
html+=`</tbody></table></div></div>`;

html+='</div>'; // close report-area
document.getElementById('rep-content').innerHTML=html;
window._reportData={genStats,byDev,bySys,timeline,productivity,sysHealth,dateFrom,dateTo,ov,sla,working:_workAll};
...
if(filteredProd.length){chartInstances.prod=new Chart(document.getElementById('ch-prod'),{type:'bar',data:{labels:filteredProd.map(p=>p.name),datasets:[{label:'Concluídas',data:filteredProd.map(p=>p.concluidas||0),backgroundColor:'#10b981',borderRadius:4},{label:'Aguardando Aceite',data:filteredProd.map(p=>p.aguardando||p.em_aberto||0),backgroundColor:'#3b82f6',borderRadius:4}]},options:{...co,scales:{x:{stacked:true,grid:{display:false},ticks:{color:tc,font:{size:10}}},y:{stacked:true,grid:{color:gc},ticks:{color:tc}}}}})}
...
if(devDays.length){chartInstances.avgdays=new Chart(document.getElementById('ch-avgdays'),{type:'bar',data:{labels:devDays.map(d=>d.name),datasets:[{label:'Minutos',data:devDays.map(d=>Math.round(parseFloat(d.avg_days)*24*60)),backgroundColor:devDays.map(d=>d.avatar_color||'#3b82f6'),borderRadius:4}]},options:{responsive:true,plugins:{legend:{display:false},tooltip:{callbacks:{label:function(ctx){const v=ctx.raw;return v<60?v+'min':Math.floor(v/60)+'h '+v%60+'m'}}}},scales:{x:{grid:{color:gc},ticks:{color:tc}},y:{grid:{display:false},ticks:{color:lc,font:{size:11},callback:function(v){return v<60?v+'min':Math.floor(v/60)+'h'+(v%60?' '+v%60+'m':'')}}}}}})}
...
const _xlAvgH=rd.ov.avg_days?parseFloat(rd.ov.avg_days)*24:0;const _xlAvgTxt=_xlAvgH?_xlAvgH<1?Math.round(_xlAvgH*60)+'min':_xlAvgH.toFixed(1)+'h':'—';const ov1=[['Métrica','Valor'],['Total',rd.ov.total||0],['Concluídas',rd.ov.concluidas||0],['Ativas',rd.ov.ativas||0],['Taxa Conclusão %',rd.ov.total?Math.round((rd.ov.concluidas/rd.ov.total)*100):0],['Tempo Médio',_xlAvgTxt],['',''],['Período',rd.dateFrom+' a '+rd.dateTo]];
...
if(selectedCharts.indexOf('bysys')>=0){
  const sr=[['Sistema','Total','Abertas','Andamento','Concluídas','Tempo Médio']];
...
const _pdfAvgH=ov.avg_days?parseFloat(ov.avg_days)*24:0;const _pdfAvgTxt=_pdfAvgH?_pdfAvgH<1?Math.round(_pdfAvgH*60)+'min':_pdfAvgH.toFixed(1)+'h':'—';
const kpis=[
...
if(selectedCharts.indexOf('bysys')>=0){
  ...
  const bsh=['Sistema','Total','Abertas','Andamento','Concluídas','Tempo Médio'];
  const bsw=[72,22,22,26,26,30];
  // Header row
  doc.setFillColor(30,45,80);doc.roundedRect(mg,y,tblW,6.5,1,1,'F');
  doc.setTextColor(220,225,240);doc.setFontSize(7.5);doc.setFont('helvetica','bold');
  ...
  filteredBySys.forEach((s,ri)=>{
    safeY(7);
    if(ri%2===0){doc.setFillColor(240,242,248);doc.rect(mg,y,tblW,6,'F')}
    const _pdfSysMin=s.avg_days?Math.round(parseFloat(s.avg_days)*24*60):0;const _pdfSysTxt=_pdfSysMin?(_pdfSysMin<60?_pdfSysMin+'min':Math.floor(_pdfSysMin/60)+'h '+(_pdfSysMin%60)+'m'):'—';const vals=[s.name||'Sem sistema',String(s.total||0),String(s.abertas||0),String(s.andamento||0),String(s.concluidas||0),_pdfSysTxt];
    doc.setTextColor(40,50,70);doc.setFontSize(7.5);
...
// ============================================================
// TABELA: PRÓXIMAS DEMANDAS
// ============================================================
const _aguardPdf = rd.working ? [] : [];
// Buscar diretamente do _aguardList se disponível, senão usar dados já carregados
const _proxList = window._aguardList || [];

if(_proxList.length > 0 && selectedCharts.indexOf('bysys') >= 0 || true) {
  const minSpacing = 15;
  y += minSpacing;
  if(y + 50 > ph - 16){ newPg(); }
  ...
  doc.text('PRÓXIMAS DEMANDAS', mg, y); y += 7;

  // Header
  const ph2 = ['Demanda', 'Sistema', 'Dev(s)', 'Prior.', 'Prazo'];
  const pw2 = [82, 34, 34, 24, 24];
  doc.setFillColor(30, 45, 80);
  doc.roundedRect(mg, y, tblW, 6.5, 1, 1, 'F');
  doc.setTextColor(220, 225, 240);
  doc.setFontSize(7.5);
  doc.setFont('helvetica', 'bold');
  ph2.forEach((h, i) => {
    let cx = mg;
    for(let j = 0; j < i; j++) cx += pw2[j];
    doc.text(h, cx + 3, y + 4.5);
  });
  y += 7.5;

  if(_proxList.length === 0) {
    doc.setFont('helvetica', 'italic');
    doc.setFontSize(8);
    doc.setTextColor(140, 150, 170);
    doc.text('Nenhuma demanda aguardando aceite', pw / 2, y + 4, { align: 'center' });
    y += 10;
  } else {
    doc.setFont('helvetica', 'normal');
    _proxList.forEach((d, ri) => {
      safeY(7);
      if(ri % 2 === 0) { doc.setFillColor(240, 242, 248); doc.rect(mg, y, tblW, 6, 'F'); }
      doc.setTextColor(40, 50, 70);
      doc.setFontSize(7.5);

      const title = (d.title || '').substring(0, 50) + (d.title && d.title.length > 50 ? '...' : '');
      const sys   = (d.system_name || '—').substring(0, 18);
      const pri   = d.priority || '—';
      const devs  = (d.devs || []).map(dv => dv.name).join(', ').substring(0, 20) || '—';
      let dl = '—';
      if(d.deadline) {
        const diff = Math.ceil((new Date(d.deadline + 'T12:00:00') - Date.now()) / 86400000);
        dl = d.deadline.split('-').reverse().join('/');
        if(diff < 0) { doc.setTextColor(220, 38, 38); dl += ' (' + Math.abs(diff) + 'd atr)'; }
        else if(diff <= 3) { doc.setTextColor(220, 38, 38); }
        else if(diff <= 7) { doc.setTextColor(234, 88, 12); }
      }

      const vals = [title, sys, pri, devs, dl];
      let cx = mg;
      vals.forEach((v, i) => {
        doc.setTextColor(40, 50, 70);
        if(i === 3 && d.deadline) {
...
const _avgRaw=st.avg_completion_days?parseFloat(st.avg_completion_days):null;const avgDays=(_avgRaw&&!isNaN(_avgRaw)&&_avgRaw>0)?(()=>{const mins=Math.round(_avgRaw*24*60);if(mins<60)return mins+'min';const h=Math.floor(mins/60);const m=mins%60;return h+'h'+(m?' '+m+'m':'')})():'—';
...
html+=`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(90px,1fr));gap:8px;margin-bottom:20px">
  ...
  <div class="sc" style="border-left:3px solid #f59e0b"><div class="sc-l">Tempo Médio</div><div class="sc-v">${avgDays}</div></div>
...
// ============================================================
// PATCH: Ponto Digital — Timer ao vivo + Ícones + Folgas
// ============================================================

// ── 1. TIMER AO VIVO ────────────────────────────────────────
let _pontoLiveTimer   = null;
let _pontoActiveSince = null;
let _pontoWorkedBase  = 0;
window._pontoJornada  = 28800;

function _pontoFmt(s){const h=Math.floor(s/3600),m=Math.floor((s%3600)/60),sc=s%60;return h+'h '+String(m).padStart(2,'0')+'m '+String(sc).padStart(2,'0')+'s';}

function startPontoLiveTimer(base, since){
  clearInterval(_pontoLiveTimer);
  _pontoWorkedBase  = base||0;
  _pontoActiveSince = since ? new Date(since).getTime() : null;
  function tick(){
    const elH=document.getElementById('ponto-hoje-val');
    const elR=document.getElementById('ponto-restante-val');
    if(!elH){clearInterval(_pontoLiveTimer);return;}
    let total=_pontoWorkedBase;
    if(_pontoActiveSince) total+=Math.floor((Date.now()-_pontoActiveSince)/1000);
    elH.textContent=_pontoFmt(total);
    if(elR){
      const rest=Math.max(0,window._pontoJornada-total);
      elR.textContent=_pontoFmt(rest);
      elR.style.color=rest===0?'var(--err)':'var(--ok)';
    }
  }
  tick();
  _pontoLiveTimer=setInterval(tick,1000);
}

function stopPontoLiveTimer(){
  clearInterval(_pontoLiveTimer);
  _pontoLiveTimer=null;
  _pontoActiveSince=null;
}

// ── 2. ÍCONES MINIMALISTAS DE STATUS ─────────────────────────
function _injectPontoCss(){
  if(document.getElementById('ponto-patch-css'))return;
  const s=document.createElement('style');
  s.id='ponto-patch-css';
  s.textContent=`
    @keyframes ponto-ping{75%,100%{transform:scale(2);opacity:0}}
    .ponto-status-badge{display:inline-flex;align-items:center;gap:5px;padding:3px 8px;border-radius:20px;font-size:11px;font-weight:600}
    .pbs-ativo  {background:var(--okb,#d1fae5);color:var(--ok,#10b981)}
    .pbs-pausado{background:var(--bg3);color:var(--t3)}
    .pbs-folga  {background:rgba(99,102,241,.12);color:#6366f1}
  `;
  document.head.appendChild(s);
}

function pontoStatusIcon(status){
  if(status==='Ativo') return `<span class="ponto-status-badge pbs-ativo">
    <span style="position:relative;display:inline-flex;width:10px;height:10px">
      <span style="position:absolute;inset:0;border-radius:50%;background:var(--ok);opacity:.35;animation:ponto-ping 1.4s cubic-bezier(0,0,.2,1) infinite"></span>
      <svg width="8" height="8" viewBox="0 0 8 8" style="position:relative;margin:auto"><circle cx="4" cy="4" r="4" fill="var(--ok)"/></svg>
    </span>Ativo</span>`;
  if(status==='Pausado') return `<span class="ponto-status-badge pbs-pausado">
    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
      <rect x="6" y="4" width="4" height="16" rx="1"/><rect x="14" y="4" width="4" height="16" rx="1"/>
    </svg>Pausado</span>`;
  if(status==='folga'||status==='Folga') return `<span class="ponto-status-badge pbs-folga">
    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
      <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/>
    </svg>${status}</span>`;
  return `<span style="color:var(--t3)">${status||'—'}</span>`;
}

// ── 3. SISTEMA DE FOLGAS ──────────────────────────────────────
const FOLGA_TIPOS=['Folga','Atestado Médico','Feriado','Banco de Horas','Licença','Outro'];

function openFolgaModal(userId,userName,date){
  let mo=document.getElementById('m-folga');
  if(!mo){
    mo=document.createElement('div');
    mo.className='modal-o';
    mo.id='m-folga';
    mo.innerHTML=`<div class="modal" style="max-width:460px">
      <div class="modal-h"><h3>Registrar Folga / Justificativa</h3><button class="modal-x" onclick="closeM('m-folga')">×</button></div>
      <div class="modal-b">
        <input type="hidden" id="folga-user-id">
        <div class="fg2">
          <div class="fg full"><label>Colaborador</label>
            <input id="folga-user-name" readonly style="background:var(--bg3);color:var(--t2);cursor:default"></div>
          <div class="fg"><label>Data *</label><input type="date" id="folga-date"></div>
          <div class="fg"><label>Tipo *</label>
            <select id="folga-tipo">${FOLGA_TIPOS.map(t=>`<option>${t}</option>`).join('')}</select></div>
          <div class="fg full"><label>Observação</label>
            <textarea id="folga-obs" rows="2" placeholder="Ex: CID A09, consulta médica..."></textarea></div>
        </div>
      </div>
      <div class="modal-f">
        <button class="btn btn-g" onclick="closeM('m-folga')">Cancelar</button>
        <button class="btn btn-p" onclick="saveFolga()">Registrar</button>
      </div></div>`;
    document.body.appendChild(mo);
  }
  document.getElementById('folga-user-id').value  = userId||'';
  document.getElementById('folga-user-name').value= userName||'';
  document.getElementById('folga-date').value     = date||new Date().toISOString().split('T')[0];
  document.getElementById('folga-tipo').value     = 'Folga';
  document.getElementById('folga-obs').value      = '';
  openM('m-folga');
}

async function saveFolga(){
  const userId=document.getElementById('folga-user-id').value;
  const date  =document.getElementById('folga-date').value;
  const tipo  =document.getElementById('folga-tipo').value;
  const obs   =document.getElementById('folga-obs').value.trim();
  if(!date||!tipo)return alert('Preencha data e tipo');
  const r=await api('ponto_folga',{method:'POST',body:{user_id:userId,date,tipo,obs}});
  if(r?.success){showToast(IC.check+' Folga registrada');closeM('m-folga');if(getCurrentPage()==='ponto')loadPonto();}
  else alert(r?.error||'Erro ao registrar');
}

async function deleteFolga(id){
  if(!confirm('Remover essa justificativa?'))return;
  const r=await api('ponto_folga',{method:'DELETE',params:{id}});
  if(r?.success){showToast('Justificativa removida');if(getCurrentPage()==='ponto')loadPonto();}
}

// ── 4. loadPonto() REESCRITA ──────────────────────────────────
async function loadPonto(){
  _injectPontoCss();
  const c=document.getElementById('ponto-content');
  if(!c)return;

  const dateFrom=document.getElementById('ponto-from')?.value||new Date().toISOString().split('T')[0];
  const dateTo  =document.getElementById('ponto-to')?.value  ||new Date().toISOString().split('T')[0];
  const userSel =document.getElementById('ponto-user')?.value||ME?.id;
  const CAN_ADM =IS_ADMIN||(typeof IS_ADMIN_DEV!=='undefined'&&IS_ADMIN_DEV);
  const hoje    =new Date().toISOString().split('T')[0];

  const [today,history,team,users,folgas]=await Promise.all([
    api('ponto_today'),
    api('ponto_history',{params:{user_id:userSel,from:dateFrom,to:dateTo}}),
    CAN_ADM?api('ponto_team_today'):Promise.resolve([]),
    CAN_ADM?api('all_users_list') :Promise.resolve([]),
    CAN_ADM?api('ponto_folga',{params:{from:dateFrom,to:dateTo}}):Promise.resolve([])
  ]);

  const folgaMap={};
  (folgas||[]).forEach(f=>{folgaMap[f.user_id+'_'+f.date]=f;});

  const jornadaSec=(today?.jornada_hours||8)*3600;
  window._pontoJornada=jornadaSec;

  if(today?.status==='Ativo'&&today?.active_since){
    startPontoLiveTimer(today.worked_seconds||0,today.active_since);
  } else {
    stopPontoLiveTimer();
  }

  const workedSec =today?.worked_seconds||0;
  const restSec   =Math.max(0,jornadaSec-workedSec);
  const statusNow =today?.status||'Pausado';

  let html='';

  // Cards de topo
  html+=`<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:20px">
    <div class="tbl-c" style="padding:18px 20px">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--t3);margin-bottom:8px">HOJE</div>
      <div id="ponto-hoje-val" style="font-size:28px;font-weight:800;color:var(--acc);font-family:'JetBrains Mono',monospace">${_pontoFmt(workedSec)}</div>
    </div>
    <div class="tbl-c" style="padding:18px 20px">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--t3);margin-bottom:8px">RESTANTE</div>
      <div id="ponto-restante-val" style="font-size:28px;font-weight:800;color:var(--ok);font-family:'JetBrains Mono',monospace">${_pontoFmt(restSec)}</div>
    </div>
    <div class="tbl-c" style="padding:18px 20px">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--t3);margin-bottom:8px">JORNADA</div>
      <div style="font-size:28px;font-weight:800;color:var(--acc);font-family:'JetBrains Mono',monospace">${today?.jornada_hours||8}h/dia</div>
    </div>
    <div class="tbl-c" style="padding:18px 20px;border-left:3px solid ${statusNow==='Ativo'?'var(--ok)':'var(--bdr)'}">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--t3);margin-bottom:8px">STATUS</div>
      <div style="font-size:16px">${pontoStatusIcon(statusNow)}</div>
    </div>
  </div>`;

  // Filtros
  html+=`<div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;flex-wrap:wrap">
    <input type="date" class="fsel" id="ponto-from" value="${dateFrom}" onchange="loadPonto()">
    <span style="color:var(--t3);font-size:12px">até</span>
    <input type="date" class="fsel" id="ponto-to" value="${dateTo}" onchange="loadPonto()">`;
  if(CAN_ADM){
    html+=`<select class="fsel" id="ponto-user" onchange="loadPonto()">
      ${(users||[]).map(u=>`<option value="${u.id}"${u.id==userSel?' selected':''}>${esc(u.name)}</option>`).join('')}
    </select>
    <button class="btn btn-g btn-sm" style="margin-left:auto" onclick="openFolgaModal('','','${hoje}')">
      ${IC.plus} Registrar Folga
    </button>`;
  }
  html+='</div>';

  // Equipe hoje (admin)
  if(CAN_ADM&&(team||[]).length>0){
    html+=`<div class="tbl-c" style="margin-bottom:20px">
      <div class="tbl-bar" style="padding:14px 16px"><h3 style="font-size:13px;font-weight:700">Equipe Hoje</h3></div>
      <div style="overflow-x:auto"><table>
        <thead><tr>
          <th>DEV</th><th>STATUS</th><th>TRABALHADO</th><th>JORNADA</th><th>PROGRESSO</th>
          <th style="width:80px">AÇÕES</th>
        </tr></thead><tbody>`;
    team.forEach(m=>{
      const fk=m.user_id+'_'+hoje;
      const folga=folgaMap[fk];
      const jSec=(m.jornada_hours||8)*3600;
      const wSec=m.worked_seconds||0;
      const pct=Math.min(100,Math.round((wSec/jSec)*100));
      const pColor=pct>=100?'var(--ok)':pct>=50?'var(--acc)':'var(--bdr)';
      html+=`<tr>
        <td><div style="display:flex;align-items:center;gap:8px">${av(m.name,m.avatar_color,24,m.avatar_file||'',m.role||'')}
          <span style="font-weight:600;font-size:13px">${esc(m.name)}</span></div></td>
        <td>${folga?`<span class="ponto-status-badge pbs-folga">${esc(folga.tipo)}</span>`:pontoStatusIcon(m.status)}</td>
        ...
        <td>${m.jornada_hours||8}h</td>
        <td style="min-width:140px">${folga
          ?`<span style="font-size:11px;color:#6366f1;font-style:italic">${folga.obs?esc(folga.obs):'Justificado'}</span>`
          :`<div style="display:flex;align-items:center;gap:8px">
              <div style="flex:1;height:5px;background:var(--bg4);border-radius:3px;overflow:hidden">
                <div style="height:100%;width:${pct}%;background:${pColor};border-radius:3px;transition:width .5s"></div>
              </div>
              <span style="font-size:11px;font-weight:700;color:${pColor};min-width:30px">${pct}%</span>
            </div>`
        }</td>
        <td style="text-align:center">${folga
          ?`<button class="btn btn-g btn-sm" style="color:var(--err)" onclick="deleteFolga(${folga.id})" title="Remover">${IC.trash}</button>`
          :`<button class="btn btn-g btn-sm" onclick="openFolgaModal(${m.user_id},'${esc(m.name)}','${hoje}')" title="Registrar folga">${IC.plus}</button>`
        }</td>
      </tr>`;
    });
    html+=`</tbody></table></div></div>`;
  ...
  // Histórico
  html+=`<div class="tbl-c">
    <div class="tbl-bar" style="padding:14px 16px"><h3 style="font-size:13px;font-weight:700">Histórico de Ponto</h3></div>
    <div style="overflow-x:auto"><table>
      <thead><tr>
        <th>DATA</th><th>SESSÕES</th><th>TOTAL</th><th>JORNADA</th><th>SALDO</th>
        ${CAN_ADM?'<th>TIPO</th>':''}
        ${CAN_ADM?'<th>AÇÕES</th>':''}
      </tr></thead><tbody>`;

  (history||[]).forEach(row=>{
    const fk=(row.user_id||userSel)+'_'+row.date;
    const folga=folgaMap[fk];
    const saldo=row.balance_seconds||0;
    const saldoFmt=(saldo<0?'-':'+')+_pontoFmt(Math.abs(saldo));
    const sColor=folga?'#6366f1':saldo<0?'var(--err)':'var(--ok)';
    const sess=(row.sessions||[]).map(s=>{
      const f=s.clock_in?s.clock_in.slice(11,16):'—';
      const t=s.clock_out?s.clock_out.slice(11,16):'ativo';
      return f+'→'+t;
    }).join(', ')||'—';

    html+=`<tr>
      <td style="font-weight:600;font-family:'JetBrains Mono',monospace;font-size:12px">${fmtDate(row.date)}</td>
      <td style="font-size:11px;color:var(--t2)">${folga?'—':sess}</td>
      <td style="font-family:'JetBrains Mono',monospace;color:var(--acc)">${folga?'—':_pontoFmt(row.worked_seconds||0)}</td>
      <td>${row.jornada_hours||8}h</td>
      <td style="font-weight:700;color:${sColor}">
        ${folga?`<span style="font-size:11px;font-style:italic">${esc(folga.tipo)}</span>`:saldoFmt}
      </td>
      ${CAN_ADM?`<td>${folga
        ?`<span class="ponto-status-badge pbs-folga">${esc(folga.tipo)}</span>${folga.obs?`<span style="font-size:10px;color:var(--t3);display:block">${esc(folga.obs)}</span>`:''}`
        :'<span style="color:var(--t3);font-size:11px">—</span>'
      }</td>`:''}
      ${CAN_ADM?`<td style="display:flex;gap:4px;align-items:center">
        ${CAN_ADM?(folga
          ?`<button class="btn btn-g btn-sm" style="color:var(--err)" onclick="deleteFolga(${folga.id})" title="Remover folga">${IC.trash}</button>`
          :`<button class="btn btn-g btn-sm" onclick="openFolgaModal(${userSel},'','${row.date}')" title="Registrar folga">${IC.plus}</button>`
        ):''}
        ${CAN_ADM?`<button class="btn btn-g btn-sm" onclick="editPontoSession()" title="Editar">${IC.edit}</button>`:''}
      </td>`:'<td></td>'}
    </tr>`;
  });

  if(!(history||[]).length){
    html+=`<tr><td colspan="7"><div class="empty" style="padding:30px"><p>Nenhum registro no período</p></div></td></tr>`;
  }
...
// ── Registrar ponto no loadPage e pageInfo ────────────────────
// (feito automaticamente abaixo via patch de string)

// ── TIMER CHIP NO TOPBAR ─────────────────────────────────────
// Mostra o tempo trabalhado hoje no header, sempre visível
function _renderTopbarTimer(workedSec, jornadaSec, status, activeSince) {
  let el = document.getElementById('ponto-header-timer');
  if (!el) {
    el = document.createElement('div');
    el.id = 'ponto-header-timer';
    el.title = 'Ponto Digital — clique para opções, arraste para mover';

    // Posição salva ou padrão (canto superior direito)
    const saved = JSON.parse(localStorage.getItem('ponto_widget_pos') || 'null');
    const defaultRight = 20;
    const defaultTop   = 18;

    el.style.cssText = `
      position:fixed;
      top:${saved ? saved.top + 'px' : defaultTop + 'px'};
      ${saved ? 'left:' + saved.left + 'px' : 'right:' + defaultRight + 'px'};
      z-index:9998;
      display:inline-flex;align-items:center;gap:6px;
      padding:6px 12px;border-radius:20px;cursor:grab;
    ...
    document.addEventListener('mouseup', () => {
      ...
      el.style.boxShadow = '0 2px 12px rgba(0,0,0,.4)';
      // Salvar posição
      localStorage.setItem('ponto_widget_pos', JSON.stringify({
        left: parseInt(el.style.left),
        top:  parseInt(el.style.top)
      }));
    });

    // ── Click (só se não arrastou) ─────────────────────────
    ...
    el.onmouseenter = () => { if (!dragging) el.style.borderColor = 'var(--acc)'; };
    el.onmouseleave = () => { if (!dragging) el.style.borderColor = 'var(--bdr)'; };

    document.body.appendChild(el);
  }

  _updateTopbarTimer(workedSec, jornadaSec, status, activeSince);
}
...
async function handleChipAction(action) {
  document.getElementById('ponto-chip-popup')?.remove();
  if (action === 'out') {
...
function _updateTopbarTimer(workedSec, jornadaSec, status, activeSince) {
  const el = document.getElementById('ponto-header-timer');
  if (!el) return;

  // isAtivo: usar _topbarActiveSince como fonte de verdade
  const isAtivo = !!_topbarActiveSince || status === 'Ativo';
  const pct = Math.min(100, Math.round(((workedSec) / jornadaSec) * 100));

  const dotColor = isAtivo ? 'var(--ok)' : 'var(--t3)';
  const dotAnim  = isAtivo ? 'animation:ponto-ping 1.4s cubic-bezier(0,0,.2,1) infinite' : '';

  el.innerHTML = `
    <span style="position:relative;display:inline-flex;width:8px;height:8px">
      <span style="position:absolute;inset:0;border-radius:50%;background:${dotColor};opacity:.35;${dotAnim}"></span>
      <svg width="6" height="6" viewBox="0 0 6 6" style="position:relative;margin:auto">
        <circle cx="3" cy="3" r="3" fill="${dotColor}"/>
      </svg>
    </span>
    <span id="ponto-topbar-val" style="color:${isAtivo?'var(--ok)':'var(--t2)'}">${_pontoFmt(workedSec)}</span>
    <span style="color:var(--t3);font-weight:400;font-size:10px">${pct}%</span>`;
}

// Timer tick do topbar (independente da página)
...
function startTopbarTimer(base, since, jornada) {
  ...
  _topbarWorkedBase  = base || 0;
  _topbarActiveSince = since ? new Date(since).getTime() : null;
  _topbarJornada     = jornada || 28800;

  function tick() {
    let total = _topbarWorkedBase;
    if (_topbarActiveSince) total += Math.floor((Date.now() - _topbarActiveSince) / 1000);
    const pct = Math.min(100, Math.round((total / _topbarJornada) * 100));

    // Atualizar widget via innerHTML para garantir cor do dot
    const elWidget = document.getElementById('ponto-header-timer');
    if (elWidget) {
      const isAtivo = !!_topbarActiveSince;
      const dotColor = isAtivo ? 'var(--ok)' : 'var(--t3)';
      const dotAnim  = isAtivo ? 'animation:ponto-ping 1.4s cubic-bezier(0,0,.2,1) infinite' : '';
      elWidget.innerHTML = `
        <span style="position:relative;display:inline-flex;width:8px;height:8px">
        ...
        <span id="ponto-topbar-val" style="color:${isAtivo?'var(--ok)':'var(--t2)'}">${_pontoFmt(total)}</span>
        <span style="color:var(--t3);font-weight:400;font-size:10px">${pct}%</span>`;
    }

    // Sincronizar com página ponto se estiver aberta
    const elH = document.getElementById('ponto-hoje-val');
    const elR = document.getElementById('ponto-restante-val');
    if (elH) elH.textContent = _pontoFmt(total);
    if (elR) {
      const rest = Math.max(0, _topbarJornada - total);
      elR.textContent = _pontoFmt(rest);
      elR.style.color = rest === 0 ? 'var(--err)' : 'var(--ok)';
    }
  ...
  tick();
  _topbarTimerInterval = setInterval(tick, 1000);
}

// Carregar dados do timer do topbar na inicialização

// Timer tempo real para coluna Trabalhado da equipe
let _teamTimerInterval = null;
...
async function initTopbarPontoTimer() { return; // DESATIVADO
  try {
    ...
    const jornadaSec = (today.jornada_hours || 8) * 3600;
    const workedSec  = today.worked_seconds || 0;
    const status     = today.status || 'Pausado';

    // Mostrar chip no topbar
    _renderTopbarTimer(workedSec, jornadaSec, status, today.active_since);

    // Iniciar timer tick
    if (status === 'Ativo' && today.active_since) {
      startTopbarTimer(workedSec, today.active_since, jornadaSec);
    } else {
      startTopbarTimer(workedSec, null, jornadaSec);
    }
  } catch(e) {
    // silencioso — ponto pode não estar implementado ainda
  }
}
